...
当前用户决策优先级：暂停新的外部搜索。三轮全节点搜索和第四轮 4/18 检查点作为历史证据保留；除非用户再次明确授权，不得继续第四轮剩余搜索。首批原始资料的人工语义审核、覆盖证明、保守去重、知识图谱、学习视图、问题库存、章节和严格验收均已完成。当前主任务是维护范围受限正式版本，按审核日期复核动态产品/API 内容；两个无来源库存占位不得写成正式结论。
