...
2. `Chunk Size`、`Overlap` 和父子映射必须与召回、上下文完整性、索引规模和延迟一起评估，不能使用脱离数据的固定经验值。
...
**验证与追问**：固定数据快照，比较不同 `Chunk Size`、`Overlap` 和策略的 Recall、MRR、引用完整性和 P99；追问：平均长度满足约束但长尾超限时如何处理？
...
**验证与追问**：对每种方法运行相同查询集和显著性比较，记录 Recall、nDCG、MRR、Token 消耗和 P99；追问：如何区分切分收益与 Embedding 模型变化？
...
**方案、选择依据与实现**：父子 ID 带租户和版本前缀；增量更新采用版本化索引和原子别名切换，检索时先做租户过滤再回溯父片段。实现引用 `IMP-RAG-0005`，选择依据为隔离强度、回滚能力、P99 和维护成本。

**验证与追问**：压测租户隔离、版本迁移、删除传播和回滚；检查父子命中不会跨租户，比较更新前后 Recall、引用完整性和 P99。追问：共享父片段如何证明不存在隐式数据泄露？
