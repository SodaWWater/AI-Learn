...
**方案、选择依据与实现**：以文档版本和删除标记驱动增量流水线；用蓝绿索引（Blue-green Index）或版本别名完成原子切换；在检索前执行租户和文档级权限过滤。选择取决于 P99 延迟（P99 Latency）、回滚窗口、租户隔离强度和重建成本；实现仅引用 `IMP-RAG-0005`。

**验证与追问**：对新增、修改、删除和模型升级分别做幂等性（Idempotency）、回滚和权限回归；记录解析失败率、索引滞后、过滤后召回和 P99 延迟（P99 Latency）。追问：如何证明缓存没有绕过权限？如何在部分租户失败时恢复？
...
**方案、选择依据与实现**：按租户规模和风险选择物理隔离、逻辑隔离或混合分片；通过版本化索引、双写（Dual Write）和原子别名切换完成无停机迁移；权限过滤字段随请求身份传播到每次检索。选择依据是隔离强度、P99 延迟（P99 Latency）、成本、恢复时间目标和运维复杂度；实现引用 `IMP-RAG-0005`。

**验证与追问**：压测租户倾斜、迁移回滚、备份恢复、删除传播和越权样本；分层报告检索召回、P99 延迟（P99 Latency）、成本和权限错误率。追问：单租户热点如何迁移？旧模型索引何时安全下线？
